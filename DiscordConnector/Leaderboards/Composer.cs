...
internal class Composer : Base
{
	...
	public Composer(int leaderBoard) => this.leaderBoardIdx = leaderBoard;
	...
	public override void SendLeaderBoard()
	{
		try
		{
			...
			{
				DiscordConnectorPlugin.StaticLogger.LogInfo($"Starting to build leaderboard {this.leaderBoardIdx}");
			}

			if (this.leaderBoardIdx > DiscordConnectorPlugin.StaticConfig.LeaderBoards.Length || this.leaderBoardIdx < 0)
			{
				DiscordConnectorPlugin.StaticLogger.LogWarning(
					$"Tried to get leader board out of index bounds (index:{this.leaderBoardIdx}, length:{DiscordConnectorPlugin.StaticConfig.LeaderBoards.Length})");
				return;
			}

			var settings = DiscordConnectorPlugin.StaticConfig.LeaderBoards[this.leaderBoardIdx];
			var ev = DiscordConnectorPlugin.StaticConfig.LeaderBoards[this.leaderBoardIdx].WebhookEvent;

			if (DiscordConnectorPlugin.StaticConfig.DebugLeaderboardOperations)
			{
				DiscordConnectorPlugin.StaticLogger.LogInfo(
					$"Leaderboard {this.leaderBoardIdx} settings: Enabled={settings.Enabled}, Webhook Event={ev}");
			}
			...
			if (!settings.Enabled)
			{
				if (DiscordConnectorPlugin.StaticConfig.DebugLeaderboardOperations)
				{
					DiscordConnectorPlugin.StaticLogger.LogInfo($"Leaderboard {this.leaderBoardIdx} is disabled, exiting");
				}
			...
			if (DiscordConnectorPlugin.StaticConfig.DebugLeaderboardOperations)
			{
				DiscordConnectorPlugin.StaticLogger.LogInfo($"Building rankings for leaderboard {this.leaderBoardIdx}");
			}

			Dictionary<Statistic, List<CountResult>> rankings;
			try
			{
				rankings = this.makeRankings(settings);
			}
			catch (Exception ex)
			{
			...
			if (DiscordConnectorPlugin.StaticConfig.DebugLeaderboardOperations)
			{
				...
				{
					DiscordConnectorPlugin.StaticLogger.LogInfo(
						$"Rankings retrieved for leaderboard {this.leaderBoardIdx}: {string.Join(", ", rankings.Keys)}");
			...
			if (DiscordConnectorPlugin.StaticConfig.DebugLeaderboardOperations)
			{
				...
				{
					DiscordConnectorPlugin.StaticLogger.LogInfo(
						$"Built {leaderFields.Count} leader fields for leaderboard {this.leaderBoardIdx}");
			...
			{
				DiscordConnectorPlugin.StaticLogger.LogInfo(
					$"About to send leaderboard {this.leaderBoardIdx} to Discord. Embed enabled: {DiscordConnectorPlugin.StaticConfig.DiscordEmbedsEnabled}, World: {worldName}, Field count: {leaderFields.Count}");
			}
			...
			if (leaderFields.Count == 0)
			{
				...
				{
					DiscordConnectorPlugin.StaticLogger.LogWarning(
						$"No leader fields to send for leaderboard {this.leaderBoardIdx} - skipping webhook message");
				}
			...
			try
			{
				...
				if (DiscordConnectorPlugin.StaticConfig.DiscordEmbedsEnabled)
				{
					...
					if (DiscordConnectorPlugin.StaticConfig.DebugLeaderboardOperations)
					{
						DiscordConnectorPlugin.StaticLogger.LogInfo($"Sending embed for leaderboard {this.leaderBoardIdx}");
					}
					...
					try
					{
						...
						{
							DiscordConnectorPlugin.StaticLogger.LogInfo(
								$"Successfully sent embed for leaderboard {this.leaderBoardIdx}");
						}
					}
					catch (Exception ex)
					{
						DiscordConnectorPlugin.StaticLogger.LogError(
							$"Error sending leaderboard {this.leaderBoardIdx} embed: {ex.Message}");
						DiscordConnectorPlugin.StaticLogger.LogDebug($"Exception details: {ex}");
					}
				}
				else
				{
					...
					{
						DiscordConnectorPlugin.StaticLogger.LogInfo(
							$"Sending plain text message for leaderboard {this.leaderBoardIdx}");
					}
					...
					try
					{
						...
						{
							DiscordConnectorPlugin.StaticLogger.LogInfo(
								$"Successfully sent plain text message for leaderboard {this.leaderBoardIdx}");
						}
					}
					catch (Exception ex)
					{
						DiscordConnectorPlugin.StaticLogger.LogError(
							$"Error sending leaderboard {this.leaderBoardIdx} plain text: {ex.Message}");
						DiscordConnectorPlugin.StaticLogger.LogDebug($"Exception details: {ex}");
					}
				}
			}
	...
	private Dictionary<Statistic, List<CountResult>> makeRankings(LeaderBoardConfigReference settings)
	{
		...
		if (settings.TimeRange == TimeRange.AllTime)
		{
			...
			return this.AllRankings(settings);
		}
		...
		return this.TimeBasedRankings(settings, BeginEndDate.Item1, BeginEndDate.Item2);
	}
		...
		DateTime startDate, DateTime endDate)
	{
		...
		if (settings.Type == Ordering.Descending)
		{
			...
			if (settings.TimeOnline)
			{
				var timeOnlineResults = Helper.TopNResultForCategory(Categories.TimeOnline,
		...
		if (settings.Type == Ordering.Ascending)
		{
			...
			if (settings.TimeOnline)
			{
				var timeOnlineResults = Helper.BottomNResultForCategory(Categories.TimeOnline,
					settings.NumberListings, startDate, endDate);
				if (timeOnlineResults != null)
				{
					Dict.Add(Statistic.TimeOnline, timeOnlineResults);
