...
internal static class ActivePlayersAnnouncement
{
	...
	private static void SendActivePlayersBoard()
	{
		...
		var ev = Webhook.Event.ActivePlayers;
		var statsAnnouncement = "";
		if (DiscordConnectorPlugin.StaticConfig.ActivePlayersAnnouncement.IncludeCurrentlyOnline)
		{
			var currentlyOnline = CurrentOnlinePlayers();
		...
		{
			var uniqueToday = Helper.CountUniquePlayers(Categories.Join, TimeRange.Today);
		...
		{
			var uniqueThisWeek = Helper.CountUniquePlayers(Categories.Join, TimeRange.PastWeek);
		...
		{
			var uniqueAllTime = Helper.CountUniquePlayers(Categories.Join, TimeRange.AllTime);
		...
		else
		{
			...
			var formattedAnnouncement = "**Active Players**\n" + statsAnnouncement;
