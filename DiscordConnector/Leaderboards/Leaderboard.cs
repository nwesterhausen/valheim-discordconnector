...
namespace DiscordConnector
{
	internal class LeaderbBoard
	{
		...
		public LeaderbBoard()
		{
			this.LeaderBoard1 = new Composer(0);
			this.LeaderBoard2 = new Composer(1);
			this.LeaderBoard3 = new Composer(2);
			this.LeaderBoard4 = new Composer(3);
			this.LeaderBoard5 = new Composer(4);
		}
		...
		public static string RankedSecondsToString(List<CountResult> rankings)
		{
			...
			for (var i = 0; i < rankings.Count; i++)
			{
				var formattedDuration = ToReadableString(TimeSpan.FromSeconds(rankings[i].Count));
		...
		public static Tuple<DateTime, DateTime> StartEndDatesForTimeRange(TimeRange timeRange)
		{
			switch (timeRange)
			{
				...
				case TimeRange.PastWeek:
					var weekAgo = DateTime.Today.AddDays(-7.0);
				...
				case TimeRange.WeekSundayToSaturday:
					...
					var dow = (int)today2.DayOfWeek;

					var sunday = today2.AddDays(-dow);
					var saturday = today2.AddDays(6 - dow);
