...
public class Webhook
{
	...
	public static List<Event> StringToEventList(string configEntry)
	{
		...
		var cleaned = Regex.Replace(configEntry, @"[^;\w]", "");
		...
		foreach (var ev in cleaned.Split(';'))
		{
...
internal class WebhookEntry
{
	...
	{
		this.Url = url;
		this.FireOnEvents = [Webhook.Event.ALL];
	}
		...
		string avatarOverride = "", string whichWebhook = "")
	{
		...
		{
			this.FireOnEvents = fireOnEvents;
		}
	...
	public bool HasUsernameOverride() => !string.IsNullOrEmpty(this.UsernameOverride);

	/// <summary>
	///     Check if the webhook has an avatar override
	/// </summary>
	/// <returns>True if an avatar override exists for this webhook</returns>
	public bool HasAvatarOverride() => !string.IsNullOrEmpty(this.AvatarOverride);

	internal bool HasEvent(Webhook.Event ev)
	{
		if (this.FireOnEvents.Count == 0)
		{
			return false;
		}

		if (this.FireOnEvents.Contains(Webhook.Event.ALL))
		{
			DiscordConnectorPlugin.StaticLogger.LogDebug("Webhook has 'ALL' enabled");
			return true;
		}

		if (this.FireOnEvents.Contains(Webhook.Event.PlayerAll))
		{
		...
		if (this.FireOnEvents.Contains(Webhook.Event.PlayerFirstAll))
		{
		...
		if (this.FireOnEvents.Contains(Webhook.Event.EventLifecycle))
		{
		...
		if (this.FireOnEvents.Contains(Webhook.Event.ServerLifecycle))
		{
		...
		if (this.FireOnEvents.Contains(Webhook.Event.LeaderboardsAll))
		{
		...
		return this.FireOnEvents.Contains(ev);
	}
}
