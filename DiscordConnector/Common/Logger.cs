...
public sealed class VdcLogger
{
	...
	public void SetLogLevel(bool logDebugMessages) => this._logDebugMessages = logDebugMessages;
	...
	private static void InitializeLogFile()
	{
		...
		{
			var olderLogFile = $"{s_logFilePath}.{i}";
			var newerLogFile = $"{s_logFilePath}.{i - 1}";
	...
	{
		await this.LogToFileAsync("DEBUG", message);
	...
	{
		await this.LogToFileAsync("INFO", message);
		s_logger.LogInfo(message);
	}

	public async Task LogWarningAsync(string message)
	{
		await this.LogToFileAsync("WARNING", message);
		s_logger.LogWarning(message);
	}

	public async Task LogErrorAsync(string message)
	{
		await this.LogToFileAsync("ERROR", message);
		s_logger.LogError(message);
	}

	public async Task LogFatalAsync(string message)
	{
		await this.LogToFileAsync("FATAL", message);
		s_logger.LogFatal(message);
	}

	public void LogDebug(string message) => this.LogDebugAsync(message).GetAwaiter().GetResult();

	public void LogInfo(string message) => this.LogInfoAsync(message).GetAwaiter().GetResult();

	public void LogWarning(string message) => this.LogWarningAsync(message).GetAwaiter().GetResult();

	public void LogError(string message) => this.LogErrorAsync(message).GetAwaiter().GetResult();

	public void LogFatal(string message) => this.LogFatalAsync(message).GetAwaiter().GetResult();
}
