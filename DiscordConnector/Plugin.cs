...
public class DiscordConnectorPlugin : BaseUnityPlugin
{
	...
	{
		StaticLogger = new VdcLogger(this.Logger, Path.Combine(Paths.ConfigPath, LegacyConfigPath));
		StaticConfig = new PluginConfig(this.Config);
	...
	/// </summary>
	internal static string PublicIpAddress { get; private set; } = "Checking...";
	...
	private void Awake()
	{
		...
		this._harmony = Harmony.CreateAndPatchAll(typeof(DiscordConnectorPlugin).Assembly, ModGuid);
	}
	...
	private static void InitializePublicIP() =>
		...
		Task.Run(async () =>
		{
			...
			PublicIpAddress = await PublicIPChecker.GetPublicIPAsync().ConfigureAwait(false);
			await StaticLogger.LogDebugAsync($"Public IP address determined to be {PublicIpAddress}").ConfigureAwait(false);
