...
internal class ActivePlayersAnnouncementConfig
{
	...
	public ActivePlayersAnnouncementConfigValues Value =>
		new()
		{
			...
			DisabledWhenNooneOnline = this.disableWhenNooneOnline.Value,
			PeriodInMinutes = this.periodInMinutes.Value,
			IncludeCurrentlyOnline = this.includeCurrentlyOnline.Value,
			IncludeTotalToday = this.includePlayersToday.Value,
			IncludeTotalPastWeek = this.includePlayersPastWeek.Value,
			IncludeTotalAllTime = this.includePlayersAllTime.Value
		};

	public string ConfigAsJson()
	{
		var jsonString = "{";
		jsonString += $"\"enabled\":\"{this.enabled.Value}\",";
		jsonString += $"\"disableWhenNooneOnline\":\"{this.disableWhenNooneOnline.Value}\",";
		jsonString += $"\"periodInMinutes\":{this.periodInMinutes.Value},";
		jsonString += $"\"includeCurrentlyOnline\":\"{this.includeCurrentlyOnline.Value}\",";
		jsonString += $"\"includePlayersToday\":\"{this.includePlayersToday.Value}\",";
		jsonString += $"\"includePlayersPastWeek\":\"{this.includePlayersPastWeek.Value}\",";
		jsonString += $"\"includePlayersAllTime\":\"{this.includePlayersAllTime.Value}\"";
		jsonString += "}";
		return jsonString;
	}
}
