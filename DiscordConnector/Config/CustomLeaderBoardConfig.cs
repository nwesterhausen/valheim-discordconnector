...
internal class LeaderBoardConfigValues
{
	...
	public LeaderBoardConfigValues(ConfigFile config, string header)
	{
		...
		this.displayedHeading = config.Bind<string>(header,
		...
		this.timeRange = config.Bind(header,
			TimeRangeTitle,
			TimeRangeDefault,
		...
		this.numberListings = config.Bind(header,
			NumberListingsTitle,
		...
		this.deaths = config.Bind(header,
			DeathsTitle,
			DeathsDefault,
			DeathsDescription);

		this.sessions = config.Bind(header,
			SessionsTitle,
			SessionsDefault,
			SessionsDescription);

		this.shouts = config.Bind(header,
			ShoutsTitle,
			ShoutsDefault,
			ShoutsDescription);

		this.pings = config.Bind(header,
			PingsTitle,
			PingsDefault,
			PingsDescription);

		this.timeOnline = config.Bind(header,
			TimeOnlineTitle,
			TimeOnlineDefault,
	...
	public string ConfigAsJson()
	{
		...
		jsonString += $"\"displayedHeading\":\"{this.displayedHeading.Value}\",";
		jsonString += $"\"type\":\"{this.type.Value}\",";
		jsonString += $"\"timeRange\":\"{this.timeRange.Value}\",";
		jsonString += $"\"numberListings\":{this.numberListings.Value},";
		jsonString += $"\"deaths\":\"{this.deaths.Value}\",";
		jsonString += $"\"sessions\":\"{this.sessions.Value}\",";
		jsonString += $"\"shouts\":\"{this.shouts.Value}\",";
		jsonString += $"\"pings\":\"{this.pings.Value}\",";
		jsonString += $"\"timeOnline\":\"{this.timeOnline.Value}\"";
		jsonString += "}";
		return jsonString;
	}
}
