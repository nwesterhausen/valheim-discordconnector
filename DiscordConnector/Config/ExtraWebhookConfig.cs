...
internal class ExtraWebhookConfig
{
	...
	public ExtraWebhookConfig(ConfigFile configFile)
	{
		...
		this.webhookUsernameOverrideList = [];
		this.webhookAvatarOverrideList = [];


		for (var i = 0; i < MAX_WEBHOOKS; i++)
		{
			this.webhookUrlList.Add(this.config.Bind<string>(
				EXTRA_WEBHOOKS,
				$"Webhook URL {i + 1}",
				"",
			...
			this.webhookEventsList.Add(this.config.Bind<string>(
				EXTRA_WEBHOOKS,
				$"Webhook Events {i + 1}",
				"ALL",
			...
			this.webhookUsernameOverrideList.Add(this.config.Bind<string>(
				EXTRA_WEBHOOKS,
			...
			this.webhookAvatarOverrideList.Add(this.config.Bind<string>(
				EXTRA_WEBHOOKS,
		...
		this.webhookEntries = this.LoadWebhookEntries();
	}
	...
	private List<WebhookEntry> LoadWebhookEntries()
	{
		...
		if (this.webhookUrlList.Count != this.webhookEventsList.Count || this.webhookUrlList.Count != this.webhookUsernameOverrideList.Count ||
		    this.webhookUrlList.Count != this.webhookAvatarOverrideList.Count)
		{
			DiscordConnectorPlugin.StaticLogger.LogError(
				"Webhook lists are not the same length. This should not happen.");
			DiscordConnectorPlugin.StaticLogger.LogError(
				$"URLs: {this.webhookUrlList.Count}, Events: {this.webhookEventsList.Count}, Usernames: {this.webhookUsernameOverrideList.Count}, Avatars: {this.webhookAvatarOverrideList.Count}");
			return [];
		}

		List<WebhookEntry> entries = [];

		for (var i = 0; i < this.webhookUrlList.Count; i++)
		{
			// If either the URL or the events are empty, skip this entry
			if (string.IsNullOrEmpty(this.webhookUrlList[i].Value) || string.IsNullOrEmpty(this.webhookEventsList[i].Value))
			{
			...
			var events = Webhook.StringToEventList(this.webhookEventsList[i].Value);
			...
			WebhookEntry entry = new(this.webhookUrlList[i].Value,
				events
			);

			if (!string.IsNullOrEmpty(this.webhookUsernameOverrideList[i].Value))
			{
				entry.UsernameOverride = this.webhookUsernameOverrideList[i].Value;
			}

			if (!string.IsNullOrEmpty(this.webhookAvatarOverrideList[i].Value))
			{
				entry.AvatarOverride = this.webhookAvatarOverrideList[i].Value;
			}
	...
	public string ConfigAsJson()
	{
		...
		{
			jsonString +=
				$"\"webhookURL{i + 1}\": \"{(string.IsNullOrEmpty(this.webhookUrlList[i].Value) ? "unset" : "REDACTED")}\",";
			jsonString += $"\"webhookEvents{i + 1}\": \"{this.webhookEventsList[i].Value}\",";
			jsonString += $"\"webhookUsernameOverride{i + 1}\": \"{this.webhookUsernameOverrideList[i].Value}\",";
			jsonString += $"\"webhookAvatarOverride{i + 1}\": \"{this.webhookAvatarOverrideList[i].Value}\"";
	...
	public List<WebhookEntry> GetWebhookEntries() => this.webhookEntries;
}
