...
internal static class MessageTransformer
{
	...
	{
		var customVariablesReplaced = rawMessage
	...
	private static string ReplaceDynamicVariables(string rawMessage, bool subtractOne = false)
	{
		...
		var dynamicReplacedMessage = ReplacePublicIp(rawMessage);
	...
	/// </summary>
	/// <param name="rawMessage">Raw message to format</param>
	public static string FormatServerMessage(string rawMessage) => ReplaceVariables(rawMessage);
	...
	public static string FormatPlayerMessage(string rawMessage, string playerName, string playerId,
		...
		ReplaceVariables(rawMessage, subtractOne)
	...
	public static string FormatPlayerMessage(string rawMessage, string playerName, string playerId, Vector3 pos,
		bool subtractOne = false) =>
		FormatPlayerMessage(rawMessage, playerName, playerId, subtractOne)
			.Replace(POS, $"{pos}");
	...
	public static string FormatPlayerMessage(string rawMessage, string playerName, string playerSteamId, string shout,
		...
		FormatPlayerMessage(rawMessage, playerName, playerSteamId, pos, subtractOne)
			.Replace(SHOUT, shout);
	...
	public static string FormatEventMessage(string rawMessage, string eventStartMsg, string eventEndMsg, Vector3 pos) =>
		FormatEventMessage(rawMessage, eventStartMsg, eventEndMsg)
			.Replace(POS, $"{pos}");
	...
	public static string FormatEventStartMessage(string rawMessage, string eventStartMsg, string eventEndMsg,
		Vector3 pos) =>
		FormatEventMessage(rawMessage, eventStartMsg, eventEndMsg, pos)
			.Replace(EVENT_MSG, eventStartMsg);
	...
	/// </summary>
	/// <param name="rawMessage">Raw message to format</param>
	public static string FormatLeaderBoardHeader(string rawMessage) => ReplaceVariables(rawMessage);
	...
	public static string CleanCaretFormatting(string str)
	{
		...
		var result = OpenCaretRegex.Replace(str, @"", 1);
	...
	public static string FormatVector3AsPos(Vector3 vec3) =>
		DiscordConnectorPlugin.StaticConfig.PosVarFormat
	...
	{
		var posStr = FormatVector3AsPos(vec3);
	...
	{
		var formattedMessage = FormatServerMessage(rawMessage);
		var worldName = ZNet.instance != null ? ZNet.instance.GetWorldName() : "Unknown World";
	...
	public static EmbedBuilder CreateDeathEmbed(ZNetPeer peer, string message, Webhook.Event eventType)
	{
		...
		var playerHostName = peer.m_socket.GetHostName();
		var position = peer.m_refPos;
		var worldName = ZNet.instance != null ? ZNet.instance.GetWorldName() : "Unknown World";
		var serverName = DiscordConnectorPlugin.StaticConfig.ServerName;
	...
	public static EmbedBuilder CreateShoutEmbed(ZNetPeer peer, string text, Webhook.Event eventType)
	{
		...
		var shoutText = DiscordConnectorPlugin.StaticConfig.ChatShoutAllCaps ? text.ToUpper() : text;
	...
	{
		var formattedMessage = FormatPlayerMessage(rawMessage, playerName, playerId, subtractOne);
		var worldName = ZNet.instance != null ? ZNet.instance.GetWorldName() : "Unknown World";
	...
	public static EmbedBuilder CreatePlayerMessageEmbed(string rawMessage, Webhook.Event eventType,
		...
		bool subtractOne = false)
	{
		var formattedMessage = FormatPlayerMessage(rawMessage, playerName, playerId, position, subtractOne);
		var worldName = ZNet.instance != null ? ZNet.instance.GetWorldName() : "Unknown World";
		...
		bool subtractOne = false)
	{
		...
		var formattedMessage = FormatPlayerMessage(rawMessage, playerName, playerId, shout, subtractOne);
		var worldName = ZNet.instance != null ? ZNet.instance.GetWorldName() : "Unknown World";
		...
		Vector3 position, bool subtractOne = false)
	{
		// For chat messages, we want to use the actual shout as the description
		// so we'll just process variables in the raw message
		var formattedMessage = FormatPlayerMessage(rawMessage, playerName, playerId, shout, position, subtractOne);
		var worldName = ZNet.instance != null ? ZNet.instance.GetWorldName() : "Unknown World";
	...
	{
		var formattedMessage = FormatEventMessage(rawMessage, eventStartMsg, eventEndMsg);
		var worldName = ZNet.instance != null ? ZNet.instance.GetWorldName() : "Unknown World";
		string eventName;
		...
		else
		{
			...
			DiscordConnectorPlugin.StaticLogger.LogInfo(
				$"Creating other game event embed with name: {eventName} - Event type: {eventType}");
		}
	...
	{
		var formattedMessage = FormatEventMessage(rawMessage, eventStartMsg, eventEndMsg, position);
		var worldName = ZNet.instance != null ? ZNet.instance.GetWorldName() : "Unknown World";
		string eventName;
	...
	{
		var formattedHeader = FormatLeaderBoardHeader(headerMessage, entries.Count);
		var worldName = ZNet.instance != null ? ZNet.instance.GetWorldName() : "Unknown World";

		// Extract a title from the header, limited to the first 50 characters
		var title = "Leaderboard";
		var description = formattedHeader;
	...
	/// <returns>A configured EmbedBuilder instance</returns>
	public static EmbedBuilder CreatePositionEmbed(string rawMessage, string playerName, Vector3 position,
