...
internal class DiscordApi
{
		...
		try
		{
			if (DiscordConnectorPlugin.StaticConfig.DiscordEmbedsEnabled)
			{
				try
				{
					...
					var embedBuilder = MessageTransformer.CreatePositionEmbed(message,
		...
		try
		{
			...
			if (DiscordConnectorPlugin.StaticConfig.DiscordEmbedsEnabled)
			{
				try
				{
					...
					var embedBuilder = MessageTransformer.CreateServerMessageEmbed(message, ev);
	...
	public static void SendEmbed(Webhook.Event ev, EmbedBuilder embedBuilder)
	{
		...
		try
		{
			...
			catch (Exception ex)
			{
				...
				var fallbackContent = "Error creating embed message. Please check logs.";
				try
				{
			...
			try
			{
				...
				var hasContent = !string.IsNullOrEmpty(embed.title) ||
				...
				if (!isValid)
				{
					...
					// Try to extract just the description for a fallback message
					var fallbackContent = embedBuilder.GetDescriptionForFallback() ?? string.Empty;
	...
	public static void SendSerializedJson(Webhook.Event ev, string serializedJson)
	{
		...
		foreach (var webhook in DiscordConnectorPlugin.StaticConfig.ExtraWebhooks)
		{
	...
	private static async void DispatchRequest(WebhookEntry webhook, string serializedJson)
	{
		...
		var requestId = GuidHelper.GenerateShortHexGuid();
		...
		try
		{
			var response = await request.PostAsync(webhook.Url, content);
			DiscordConnectorPlugin.StaticLogger.LogDebug(
	...
	/// <param name="message">A string optionally formatted with Discord-approved markdown syntax.</param>
	/// <param name="pos">A 3-dimensional vector representing a position</param>
	public static void SendMessage(string message, Vector3 pos) => SendMessage(Webhook.Event.Other, message, pos);

	/// <summary>
	///     Sends a <paramref name="message" /> to Discord.
	/// </summary>
	/// <param name="message">A string optionally formatted with Discord-approved markdown syntax.</param>
	public static void SendMessage(string message) => SendMessage(Webhook.Event.Other, message);
	...
	/// </summary>
	/// <param name="serializedJson">Body data for the webhook as JSON serialized into a string</param>
	private static void SendSerializedJson(string serializedJson) => SendSerializedJson(Webhook.Event.Other, serializedJson);
}
...
internal class DiscordExecuteWebhook
{
	...
	public bool IsValid() => !string.IsNullOrEmpty(this.content) || this.embeds != null;
	...
	public void ResetOverrides()
	{
		...
		{
			this.SetUsername(DiscordConnectorPlugin.StaticConfig.DefaultWebhookUsernameOverride);
		}
	}
	...
	public void SendFor(Webhook.Event ev)
	{
		...
		try
		{
			...
			{
				DiscordConnectorPlugin.StaticLogger.LogDebug($"Sending {ev} message to Primary Webhook");
				var primaryWebhook = DiscordConnectorPlugin.StaticConfig.PrimaryWebhook;
				this.ResetOverrides();

				if (primaryWebhook.HasUsernameOverride())
				{
					this.SetUsername(primaryWebhook.UsernameOverride);
				}

				if (primaryWebhook.HasAvatarOverride())
				{
					this.SetAvatarUrl(primaryWebhook.AvatarOverride);
				}
			...
			{
				DiscordConnectorPlugin.StaticLogger.LogDebug($"Sending {ev} message to Secondary Webhook");
				var secondaryWebhook = DiscordConnectorPlugin.StaticConfig.SecondaryWebhook;
				this.ResetOverrides();

				if (secondaryWebhook.HasUsernameOverride())
				{
					this.SetUsername(secondaryWebhook.UsernameOverride);
				}

				if (secondaryWebhook.HasAvatarOverride())
				{
					this.SetAvatarUrl(secondaryWebhook.AvatarOverride);
				}
			...
			foreach (var webhook in DiscordConnectorPlugin.StaticConfig.ExtraWebhooks)
			{
				if (webhook.HasEvent(ev))
				{
					...
					{
						this.SetUsername(webhook.UsernameOverride);
					}

					if (webhook.HasAvatarOverride())
					{
						this.SetAvatarUrl(webhook.AvatarOverride);
					}
...
internal class AllowedMentions
{
	public AllowedMentions()
	{
		...
		{
			this.AllowRoles(DiscordConnectorPlugin.StaticConfig.AllowedRoleMentions);
		}

		if (DiscordConnectorPlugin.StaticConfig.AllowedUserMentions.Count > 0)
		{
			this.AllowUsers(DiscordConnectorPlugin.StaticConfig.AllowedUserMentions);
		}
	}
