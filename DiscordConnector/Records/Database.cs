...
/// <summary>
///     The database class holds the connection to the database, and so all the database-accessing methods are contained
///     within.
/// </summary>
internal class Database
{
	...
	///     This will set up the database path in a default location:
	///     `BepInEx/Config/{DiscordConnectorPlugin.ModName}/records.db`
	/// </summary>
	...
	public Database(string rootStorePath)
	{
		...
		var oldDatabase = Path.Combine(Paths.ConfigPath, $"{DiscordConnectorPlugin.LegacyModName}-records.db");
	...
	public void Initialize() =>
		Task.Run(() =>
		{
			try
			{
				...
				this.DeathCollection = this.db.GetCollection<SimpleStat>("deaths");
				this.JoinCollection = this.db.GetCollection<SimpleStat>("joins");
				this.LeaveCollection = this.db.GetCollection<SimpleStat>("leaves");
				this.ShoutCollection = this.db.GetCollection<SimpleStat>("shouts");
				this.PingCollection = this.db.GetCollection<SimpleStat>("pings");
				this.PlayerToNameCollection = this.db.GetCollection<PlayerToName>("player_name");

				// Ensure indices on the collections
				Task.Run(() =>
				{
					this.EnsureIndicesOnCollections();
	...
	{
		this.DeathCollection.EnsureIndex(x => x.PlayerId);
		this.DeathCollection.EnsureIndex(x => x.Name);
		this.JoinCollection.EnsureIndex(x => x.PlayerId);
		this.JoinCollection.EnsureIndex(x => x.Name);
		this.LeaveCollection.EnsureIndex(x => x.PlayerId);
		this.LeaveCollection.EnsureIndex(x => x.Name);
		this.ShoutCollection.EnsureIndex(x => x.PlayerId);
		this.ShoutCollection.EnsureIndex(x => x.Name);
		this.PingCollection.EnsureIndex(x => x.PlayerId);
		this.PingCollection.EnsureIndex(x => x.Name);
		this.PlayerToNameCollection.EnsureIndex(x => x.PlayerId);
		this.PlayerToNameCollection.EnsureIndex(x => x.CharacterName);
	}
	...
	private void InsertSimpleStatRecord(ILiteCollection<SimpleStat> collection, string playerName,
		string playerHostName, Vector3 pos) =>
		Task.Run(() =>
		{
			SimpleStat newRecord = new(
				playerName,
				playerHostName,
				pos.x, pos.y, pos.z
			);
	...
	private void EnsurePlayerNameRecorded(string characterName, string playerHostName) =>
		Task.Run(() =>
		{
			if (this.PlayerToNameCollection.Exists(x =>
				    x.PlayerId.Equals(playerHostName) && x.CharacterName.Equals(characterName)))
			{
				// If the player record exists with both playerHostName and characterName, do nothing.
				return;
			}

			if (this.PlayerToNameCollection.Exists(x => x.PlayerId.Equals(playerHostName)))
			{
				...
				DiscordConnectorPlugin.StaticLogger.LogDebug(
					$"Multiple characters from {playerHostName}, latest is {characterName}");
			}

			// Insert the player name record if it doesn't exist
			PlayerToName newPlayer = new(characterName, playerHostName);
			this.PlayerToNameCollection.Insert(newPlayer);
		}).ConfigureAwait(false);
	...
	private void InsertSimpleStatRecord(ILiteCollection<SimpleStat> collection, string playerName,
		...
		this.InsertSimpleStatRecord(collection, playerName, playerHostName, Vector3.zero);
	...
	public void InsertSimpleStatRecord(string key, string playerName, string playerHostName, Vector3 pos)
	{
		switch (key)
		{
			case Categories.Death:
				this.InsertSimpleStatRecord(this.DeathCollection, playerName, playerHostName, pos);
				break;
			case Categories.Join:
				this.EnsurePlayerNameRecorded(playerName, playerHostName);
				this.InsertSimpleStatRecord(this.JoinCollection, playerName, playerHostName, pos);
				break;
			case Categories.Leave:
				this.InsertSimpleStatRecord(this.LeaveCollection, playerName, playerHostName, pos);
				break;
			case Categories.Ping:
				this.InsertSimpleStatRecord(this.PingCollection, playerName, playerHostName, pos);
				break;
			case Categories.Shout:
				this.InsertSimpleStatRecord(this.ShoutCollection, playerName, playerHostName, pos);
				break;
			default:
	...
	public void InsertSimpleStatRecord(string key, string playerName, string playerHostName) =>
		this.InsertSimpleStatRecord(key, playerName, playerHostName, Vector3.zero);

	/// <summary>
	///     Returns the latest known character name for the given player identifier.
	///     This first tries to find the name in the player_name table, and failing that references the join table with a
	///     query.
	...
	internal string GetLatestCharacterNameForPlayer(string playerHostName)
	{
		...
		if (this.PlayerToNameCollection.Exists(x => x.PlayerId.Equals(playerHostName)))
		{
			try
			{
				var playerInfo = this.PlayerToNameCollection
					.Query()
			...
			catch (InvalidOperationException)
			{
				...
				DiscordConnectorPlugin.StaticLogger.LogWarning(
					$"Should have found {playerHostName} in player_name table but did not!");
			}
		}
		...
		List<BsonDocument> nameQuery = this.JoinCollection.Query()
			.Where(x => x.PlayerId.Equals(playerHostName))
		...
		if (nameQuery.Count == 0)
		{
			...
			{
				DiscordConnectorPlugin.StaticLogger.LogDebug(
					$"GetLatestNameForCharacterId {playerHostName} result = NONE");
			}
		...
		{
			DiscordConnectorPlugin.StaticLogger.LogDebug(
				$"GetLatestNameForCharacterId {playerHostName} result = {result}");
		}

		Task.Run(() =>
		{
			// Insert the player name, since we didn't have it in the player_name database!
			this.EnsurePlayerNameRecorded(result["Name"], playerHostName);
		}).ConfigureAwait(false);
	...
	public List<CountResult> CountAllRecordsGrouped(string key)
	{
		...
		{
			case Categories.Death:
				return this.CountAllRecordsGrouped(this.DeathCollection);
			case Categories.Join:
				return this.CountAllRecordsGrouped(this.JoinCollection);
			case Categories.Leave:
				return this.CountAllRecordsGrouped(this.LeaveCollection);
			case Categories.Ping:
				return this.CountAllRecordsGrouped(this.PingCollection);
			case Categories.Shout:
				return this.CountAllRecordsGrouped(this.ShoutCollection);
			case Categories.TimeOnline:
				return this.TimeOnlineRecordsGrouped();
			default:
	...
	/// <param name="inclusiveStart">
	///     Whether to include the start date or not in the returned results. If true, it will use
	///     `>=` for the startDate comparison; otherwise `>`.
	/// </param>
	/// <param name="inclusiveEnd">
	///     Whether to include the end date or not in the returned results. If true, it will use `<=` for the startDate comparison; otherwise `<`.</param>
	/// <returns>List of counts with CharacterName and Total (x) for the provided SimpleStat collection.</returns>
	private List<CountResult> TimeOnlineRecordsGrouped(DateTime? startDate = null, DateTime? endDate = null,
		bool inclusiveStart = true, bool inclusiveEnd = true)
	{
		var players = this.PlayerToNameCollection.Query().ToArray();
		List<CountResult> results = new();

		foreach (var player in players)
		{
			// Create a spot to record total online time for player.
			var onlineTime = TimeSpan.FromSeconds(0.0);

			var joinsQuery = this.JoinCollection.Query().Where(x =>
				x.PlayerId.Equals(player.PlayerId) && x.Name.Equals(player.CharacterName));
			var leavesQuery = this.LeaveCollection.Query().Where(x =>
				x.PlayerId.Equals(player.PlayerId) && x.Name.Equals(player.CharacterName));

			Func<int, bool> startCompare = inclusiveStart ? x => x >= 0 : x => x > 0;
			Func<int, bool> endCompare = inclusiveEnd ? x => x <= 0 : x => x < 0;
			if (startDate != null)
			{
			...
			var joins = Array.ConvertAll(joinsQuery.ToArray(),
				stat => new JoinLeaveTime { Time = stat.Date, IsJoin = true });
			var leaves = Array.ConvertAll(leavesQuery.ToArray(),
				stat => new JoinLeaveTime { Time = stat.Date, IsJoin = false });
			var sortedJoinLeaves = joins.Concat(leaves).OrderBy(j => j.Time).ToArray();

			DiscordConnectorPlugin.StaticLogger.LogDebug(
				$"{player.PlayerId} as {player.CharacterName} has {joins.Length} joins, {leaves.Length} leaves");

			DateTime? joinedTime = null;
			foreach (var joinLeaveTime in sortedJoinLeaves)
			{
				if (joinedTime == null)
				{
					...
					{
						DiscordConnectorPlugin.StaticLogger.LogDebug(
							$"Player {player.CharacterName} left at {joinLeaveTime.Time} but was not joined.");
					}
				}
				else
				{
					...
					{
						DiscordConnectorPlugin.StaticLogger.LogDebug(
							$"Player {player.CharacterName} joined at {joinLeaveTime.Time} but was already joined at {joinedTime}");
						joinedTime = joinLeaveTime.Time;
					}
					else
					{
	...
	public int CountOfRecordsByName(string key, string playerName)
	{
		...
		{
			case Categories.Death:
				return this.CountOfRecordsByName(this.DeathCollection, playerName);
			case Categories.Join:
				return this.CountOfRecordsByName(this.JoinCollection, playerName);
			case Categories.Leave:
				return this.CountOfRecordsByName(this.LeaveCollection, playerName);
			case Categories.Ping:
				return this.CountOfRecordsByName(this.PingCollection, playerName);
			case Categories.Shout:
				return this.CountOfRecordsByName(this.ShoutCollection, playerName);
			default:
	...
	public List<CountResult> CountAllRecordsGrouped(string key, TimeRange timeRange)
	{
		...
		{
			return this.CountAllRecordsGrouped(key);
		}

		var BeginEndDate = DateHelper.StartEndDatesForTimeRange(timeRange);
		return this.CountRecordsBetweenDatesGrouped(key, BeginEndDate.Item1, BeginEndDate.Item2);
	}

	internal List<CountResult> CountRecordsBetweenDatesGrouped(string key, DateTime startDate, DateTime endDate,
		bool inclusiveStart = true, bool inclusiveEnd = true)
	{
		switch (key)
		{
			case Categories.Death:
				return this.CountAllRecordsGroupsWhereDate(this.DeathCollection, startDate, endDate, inclusiveStart,
					inclusiveEnd);
			case Categories.Join:
				return this.CountAllRecordsGroupsWhereDate(this.JoinCollection, startDate, endDate, inclusiveStart, inclusiveEnd);
			case Categories.Leave:
				return this.CountAllRecordsGroupsWhereDate(this.LeaveCollection, startDate, endDate, inclusiveStart,
					inclusiveEnd);
			case Categories.Ping:
				return this.CountAllRecordsGroupsWhereDate(this.PingCollection, startDate, endDate, inclusiveStart, inclusiveEnd);
			case Categories.Shout:
				return this.CountAllRecordsGroupsWhereDate(this.ShoutCollection, startDate, endDate, inclusiveStart,
					inclusiveEnd);
			case Categories.TimeOnline:
				return this.TimeOnlineRecordsGrouped(startDate, endDate, inclusiveStart, inclusiveEnd);
			default:
	...
	///     This looks through the provided simple stat table and counts up results for each player using the method defined in
	///     the config.
	/// </summary>
	...
	internal List<CountResult> CountAllRecordsGrouped(ILiteCollection<SimpleStat> collection)
	{
		...
		{
			DiscordConnectorPlugin.StaticLogger.LogDebug(
				$"CountAllRecordsGrouped {DiscordConnectorPlugin.StaticConfig.RecordRetrievalDiscernmentMethod}");
		}
		...
		var GroupByClause = "PlayerId";
		var SelectClause = "{Player: @Key, Count: Count(*)}";

		if (DiscordConnectorPlugin.StaticConfig.RecordRetrievalDiscernmentMethod ==
		...
		if (DiscordConnectorPlugin.StaticConfig.RecordRetrievalDiscernmentMethod ==
		    MainConfig.RetrievalDiscernmentMethods.Name)
		{
			GroupByClause = "Name";
	...
	///     Whether to include the end date or not in the returned results. If true, it will use `<=` for the startDate comparison; otherwise `<`.</param>
	/// <returns>List of counts with CharacterName and Total (x) for the provided SimpleStat collection.</returns>
	internal List<CountResult> CountAllRecordsGroupsWhereDate(ILiteCollection<SimpleStat> collection,
		DateTime startDate, DateTime endDate, bool inclusiveStart = true, bool inclusiveEnd = true)
	{
		if (DiscordConnectorPlugin.StaticConfig.DebugDatabaseMethods)
		{
			DiscordConnectorPlugin.StaticLogger.LogDebug(
				$"CountAllRecordsGroupsWhereDate {DiscordConnectorPlugin.StaticConfig.RecordRetrievalDiscernmentMethod} {startDate} {endDate}");
		}
		...
		{
			DiscordConnectorPlugin.StaticLogger.LogDebug(
				$"CountAllRecordsGroupsWhereDate {result.Count} records returned");
		}
